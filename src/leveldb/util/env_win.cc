...
std::wstring GetLastErrSzW()
{
    ...
    std::wstring Err = (SPRWSTR)lpMsgBuf;
    LocalFree(lpMsgBuf);
    return Err;
}
...
BOOL Win32RandomAccessFile::_Init( SPRWSTR path )
{
    BOOL bRet = FALSE;
    if(!_hFile)
...
BOOL Win32MapFile::_Init( SPRWSTR Path )
{
...
BOOL Win32FileLock::_Init( SPRWSTR path )
{
    BOOL bRet = FALSE;
    if(!_hFile)
